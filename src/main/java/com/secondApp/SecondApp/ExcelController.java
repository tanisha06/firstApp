        ...
        public String saveExcelToDatabase(@RequestParam("file") MultipartFile file) {
            ...
            try (Workbook workbook = new XSSFWorkbook(file.getInputStream())) {
                ...
                for (Row row : sheet) {
                    ...
                    // Skip empty rows
                    if (name.isEmpty() && ageText.isEmpty() && city.isEmpty()) continue;

                    int age = 0;
                    try {
                        if (!ageText.isEmpty()) age = Integer.parseInt(ageText);
                    } catch (NumberFormatException e) {
                        return "Invalid age at row " + (row.getRowNum() + 1);
                    }
            ...
            return "Saved " + students.size() + " records.";
        }
}
